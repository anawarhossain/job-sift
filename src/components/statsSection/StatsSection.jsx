...
export function StatsSection({ className }) {
  return (
    <section
      className={cn(
        "relative w-full bg-white dark:bg-black text-zinc-900 dark:text-white py-16 px-4 sm:px-6",
        className,
      )}
      ...
      <div className="max-w-5xl mx-auto relative z-10 text-center">
        <dl className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((stat) => (
            <div
              key={stat.id}
              className="group relative rounded-2xl border  shadow-2xl p-6
            ...
            >
              <div
                ...
                className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity
                           bg-linear-to-br from-indigo-500/4 to-transparent pointer-events-none"
              />
              <div className="flex flex-col items-center gap-4 relative z-10">
                <div
                  className="w-8 h-8 rounded-lg bg-indigo-500/50 border border-indigo-500/10
                                flex items-center justify-center group-hover:border-indigo-500/20 transition-colors"
                >
                  <StatIcon name={stat.iconName} />
                </div>
                <div className="flex flex-col gap-0.5">
                  <dd className="text-3xl sm:text-4xl font-bold tracking-tight  tabular-nums">
                    {stat.value}
                  </dd>
                  <dt className="text-xs sm:text-sm  font-medium tracking-wide">
                    {stat.label}
                  </dt>
                </div>
              </div>
